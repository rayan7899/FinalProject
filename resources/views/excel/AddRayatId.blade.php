    ...
    <div style="text-align: right !important" dir="rtl" lang="ar" class="container">
        ...
        @if (session()->has('updatedCount'))
            <div class="alert alert-info">
                تم اضافة الرقم التدريبي لـ {{ session()->get('updatedCount') }} من {{ session()->get('countOfStudents') }} متدرب
            </div>
        @endif
        @if ($errors->any())
            <div class="alert alert-danger">
        ...
        @if (session()->has('errorsArr'))
            @if (!session()->has('hasOtherMessage'))
                <div class="alert alert-danger" role="alert">
                    لم يتم اضافة الرقم التدريبي للمتدربين في الجدول ادناه بسبب وجود اخطاء في بياناتهم
                </div>
            @endif
        ...
        <div class="card">
            <div class="card-header">
                <h6> اضافة الرقم التدريبي للمتدربين المستجدين</h6>
            </div>
            <div class="card-body">
                <form id="excel_form" class="form" method="POST" action="{{ route('addRayatIdStore') }}"
