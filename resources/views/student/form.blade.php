                ...
                <table class="table table-hover table-bordered bg-white">
                    ...
                    <tbody id="pick-courses">
                        @if(isset($major_courses))
                            @forelse ($major_courses as $course)
                                <tr data-cost="{{ $course->credit_hours * 550 }}">
                                    ...
                                    <td class="text-center @if ($user->student->level < 2) d-none @endif">
                                        <input id="major_course_{{ $course->id }}"
                                               name="courses[]"
                                               type="checkbox"
                                               value="{{ $course->id }}"
                            ...
                            @empty
                                <tr><td colspan="6">لا يوجد مقررات</td></tr>
                            @endforelse
                        @else
                            <tr><td colspan="6">لا يوجد مقررات</td></tr>
                        @endif
        ...
        <form id="updateUserForm" action="{{route('UpdateOneStudent')}}" method="post" accept-charset="utf-8" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @if ($user->student->level > 1)
                    <button type="button" data-toggle="modal" data-target="#pick-courses" class="btn btn-primary">اضافة مقرارات</button>
                @endif
            </div>
