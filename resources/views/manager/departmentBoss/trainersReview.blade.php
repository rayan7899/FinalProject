...
@section('content')
                    ...
                    @if (isset($users))
                        @forelse ($users as $user)
                            ...
                            <tr id="{{ $user->national_id }}">
                                ...
                                <td class="text-center">
                                    <button class="btn btn-success px-2 py-0"
                                        onclick="showModalForm('accept','{{ $user->national_id }}','{{ $user->name }}','{{ $user->trainer->qualification }}','{{ $user->trainer->employer }}')">قبول</button>
                                    <button data-toggle="modal" data-target="#editModal" class="btn btn-primary px-2 py-0"
                                        onclick="showModalForm('edit','{{ $user->national_id }}','{{ $user->name }}','{{ $user->trainer->qualification }}','{{ $user->trainer->employer }}')">تعديل</button>
                                    <button data-toggle="modal" data-target="#editModal"
                                        onclick="showModalForm('reject','{{ $user->national_id }}','{{ $user->name }}','{{ $user->trainer->qualification }}','{{ $user->trainer->employer }}')"
                                        class="btn btn-danger px-2 py-0">رفض</button>
                                </td>
                            </tr>
                        @empty
                        @endforelse
    ...
    <script>
        ...
        function showModalForm(decision, national_id, name, qualification, employer) {
            if (event !== null) {
                ...
                case "accept":
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            sendRequest(decision, national_id, name, qualification, employer);
                        }
                    })
                    break;
                case "edit":
        ...
        function sendRequest(decision, national_id, name, qualification, employer) {
            let form = {
                        decision: decision,
            ...
            axios.post('/api/department-boss/trainers-review', form)
                .then((response) => {
                    Swal.fire({
                        position: "center",
                    ...
                    document.getElementById(form.national_id).remove();
